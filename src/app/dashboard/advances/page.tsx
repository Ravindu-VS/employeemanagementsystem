...
export default function AdvancesPage() {
  ...
  // Toggle deduction mutation
  const toggleMutation = useMutation({
    mutationFn: ({ id, deductThisWeek }: { id: string; deductThisWeek: boolean }) =>
      toggleAdvanceDeductThisWeek(id, deductThisWeek),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['advances'] });
    },
    onError: (error: any) => {
                ...
                >
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      {/* Carry Forward Toggle (only for approved, un-deducted) */}
                      {advance.status === 'approved' && !advance.isDeducted && (
                        <div className="flex items-center gap-2 mt-2 sm:mt-0">
                          <Label htmlFor={`deduct-${advance.id}`} className="text-xs text-muted-foreground whitespace-nowrap">
                            Deduct This Week
                          </Label>
                          <Button
                            variant={advance.deductThisWeek !== false ? 'default' : 'outline'}
                            size="sm"
                            className="h-7 text-xs px-2"
                            onClick={() => toggleMutation.mutate({ 
                              id: advance.id, 
                              deductThisWeek: advance.deductThisWeek === false 
                            })}
                            disabled={toggleMutation.isPending}
                          >
                            {advance.deductThisWeek !== false ? 'Yes' : 'Carry Forward'}
                          </Button>
                        </div>
                      )}
                    </div>
